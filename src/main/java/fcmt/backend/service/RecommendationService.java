...
// return
// RecommendResponseDto.builder().topArtists(List.of()).recommendedConcerts(List.of()).build();
// }
//
...
// .toList();
//
// long totalFrequencyCount = allPreferredGenres.isEmpty() ? 1 :
// allPreferredGenres.size();
//
// // 4. 공연 조회
...
// double rawScore = c.getGenres().stream().mapToDouble(g ->
// genreFrequencyMap.getOrDefault(g, 0L)).sum()
...
// .build();
// }).sorted((a, b) -> Integer.compare(b.getMatchingRate(),
// a.getMatchingRate())).toList();
//
// // 6. 결과 반환
...
// Map<String, Object> dummyPrefMap = objectMapper.readValue(resource.getInputStream(),
// new TypeReference<>() {
// });
//
